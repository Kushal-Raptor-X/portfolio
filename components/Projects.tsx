...
function LinkPill({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <a
      ...
      className="inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-white/5 px-4 py-1.5 text-xs font-medium text-neon-cyan transition-colors hover:bg-white/10 hover:text-white"
    >
      {children}
...
function ProjectCard({ project, featured }: { project: Project; featured: boolean }) {
  ...
  return (
    <article
      ...
      className={`glass group relative overflow-hidden rounded-3xl p-7 transition-colors duration-300 hover:border-white/20 ${
    ...
    >
      ...
      <div className="relative z-10">
        <div className="flex flex-wrap items-center gap-3">
          <span className="rounded-full border border-white/10 bg-white/5 px-3 py-1 font-mono text-[11px] font-medium text-neon-yellow">
            {project.badge}
          </span>
        ...
        {project.myRole && (
          <p className="mt-3 text-[15px] leading-relaxed text-gray-400">
            <span className="font-mono text-xs uppercase tracking-wider text-neon-cyan">my part → </span>
            {project.myRole}
          </p>
        )}
          ...
          <ul className="mt-4 space-y-1.5">
            {project.highlights.map((h) => (
              <li key={h} className="flex gap-2 text-sm text-gray-500">
                <span className="mt-0.5 shrink-0 font-mono text-neon-cyan/60">▸</span>
                {h}
              </li>
            ))}
          </ul>
  ...
  return (
    <section id="projects" className="relative overflow-hidden py-24 md:py-32">
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-6">
        <Reveal>
          <p data-reveal className="font-mono text-xs uppercase tracking-[0.3em] text-neon-cyan">
            // projects
          </p>
          <h2 data-reveal className="mt-4 max-w-2xl text-4xl font-medium tracking-tight text-white md:text-5xl">
            Built under pressure, <span className="font-serif italic text-neon-cyan">judged on stage.</span>
          </h2>
